...
void setup() {
  ...
  sendATCommand("AT");        // 測試連線
  sendATCommand("AT+RENEW");  // 恢復出廠設定
  sendATCommand("AT+NAMEHM10_Blue"); // 設定名稱
  sendATCommand("AT+NOTI1");  // 開啟連線通知
  sendATCommand("AT+ADDR?");  // 查詢 MAC
  sendATCommand("AT+RESET");  // 重啟模組
  delay(2000);
...
void loop() {
  ...
  char buffer[20];
  sprintf(buffer, "%2x %2x %2x %2x", mfrc522->uid.uidByte[0], mfrc522->uid.uidByte[1], mfrc522->uid.uidByte[2], mfrc522->uid.uidByte[3]);
