...
Graph readMazeCSV(const string& filename) {
    ...
    while (getline(fin, line)) {
        ...
        int idx = stoi(f[0]);
        // 強制設定權重為 3
        if (f.size() > 1 && !f[1].empty()) graph[idx].push_back({'N', stoi(f[1]), 3});
        if (f.size() > 2 && !f[2].empty()) graph[idx].push_back({'S', stoi(f[2]), 3});
        if (f.size() > 3 && !f[3].empty()) graph[idx].push_back({'W', stoi(f[3]), 3});
        if (f.size() > 4 && !f[4].empty()) graph[idx].push_back({'E', stoi(f[4]), 3});
    }
    return graph;
}
...
int main() {
    try {
        ...
        for (int i = 0; i < n; i++) {
            if (targets[i] != startNode) scores[i] = (memo_dist[startNode][targets[i]].dist / 3) * 10;
        }

        const int STEP_LIMIT = 55; 
        const int WEIGHT_LIMIT = STEP_LIMIT * 3; 
        ...
        for (int mask = 1; mask < (1 << n); ++mask) {
            for (int u = 0; u < n; ++u) {
                if (dp[mask][u] > WEIGHT_LIMIT) continue;
                for (int v = 0; v < n; ++v) {
                    if (!(mask & (1 << v))) {
                        int next_mask = mask | (1 << v);
                        int cost = memo_dist[targets[u]][targets[v]].dist;
                        if (dp[mask][u] + cost <= WEIGHT_LIMIT) {
        ...
        int maxScore = -1, bestMask = 0, lastNodeIdx = 0;
        for (int mask = 1; mask < (1 << n); mask++) {
            for (int u = 0; u < n; u++) {
                if (dp[mask][u] <= WEIGHT_LIMIT) {
                    int currentScore = 0;
        ...
        int currMask = bestMask, currNodeIdx = lastNodeIdx;
        while (currNodeIdx != -1) {
            visitOrder.push_back(currNodeIdx);
            int prev = parent[currMask][currNodeIdx];
            currMask ^= (1 << currNodeIdx);
            currNodeIdx = prev;
        }
        reverse(visitOrder.begin(), visitOrder.end());

        // --- 第二階段：清掃剩下的點 ---
        set<int> visitedIndices(visitOrder.begin(), visitOrder.end());
        int currentIdx = lastNodeIdx;
        
        while (visitedIndices.size() < (size_t)n) {
            int nearestIdx = -1;
            int minDist = 9999;
            for (int i = 0; i < n; i++) {
                if (visitedIndices.find(i) == visitedIndices.end()) {
                    if (memo_dist[targets[currentIdx]][targets[i]].dist < minDist) {
                        minDist = memo_dist[targets[currentIdx]][targets[i]].dist;
                        nearestIdx = i;
                    }
                }
            }
            if (nearestIdx != -1) {
                visitOrder.push_back(nearestIdx);
                visitedIndices.insert(nearestIdx);
                currentIdx = nearestIdx;
            }
        }

        // --- 最終指令生成 ---
        Heading h = Heading::SOUTH; // 預設朝南
        ...
        for (size_t i = 1; i < visitOrder.size(); ++i) {
            ...
            totalWeight += memo_dist[currentPos][nextPos].dist;
            
            // 標記 80 步預算在哪裡用完
            if (weightAtLimit == 0 && totalWeight > WEIGHT_LIMIT) {
                weightAtLimit = totalWeight - memo_dist[currentPos][nextPos].dist;
            }

            for (char m : memo_dist[currentPos][nextPos].dirs) {
                finalCommands += getTurnCommand(h, m);
            }
            currentPos = nextPos;
        }

        cout << "--- 優先 80 步策略 + 剩餘清掃 ---" << endl;
        cout << "前 80 步預計得分: " << maxScore << " 分" << endl;
        cout << "全部跑完總得分: " << (n-1)*10 << " 分" << endl;
        cout << "總消耗邊數: " << totalWeight / 3 << endl;
        cout << "造訪順序: ";
        for (int idx : visitOrder) cout << targets[idx] << " ";
        cout << "\n\nArduino 指令集 (複製到 vector v):\n" << finalCommands << endl;
